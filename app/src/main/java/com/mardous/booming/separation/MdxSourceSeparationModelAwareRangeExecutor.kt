...
internal class MdxSourceSeparationModelAwareRangeExecutor(
    ...
    override fun separate(
        ...
        outputDir = request.run.workDirectory,
        segmentOutputDir = request.run.segmentsDirectory,
        ...
        resumeState = request.run.resumeState,
        execution = MdxSeparationExecution(
            artifact = request.model.artifact,
            profile = request.model.executionProfile,
        ),
        expectedSourceAudioFingerprint = request.run.identity.source.audioFingerprint,
