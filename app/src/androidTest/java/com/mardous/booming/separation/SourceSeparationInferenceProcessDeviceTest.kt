...
class SourceSeparationInferenceProcessDeviceTest {
    ...
    fun validateBoundServiceStartup() {
        ...
        try {
            ...
            assertTrue(processDiagnostics.memory.vmDataBytes != null)
            assertTrue(processDiagnostics.memory.pssBytes > 0L)
            assertTrue(processDiagnostics.memory.nativePssBytes >= 0L)
            assertTrue(processDiagnostics.memory.ussBytes > 0L)
            assertTrue(processDiagnostics.memory.javaPssBytes >= 0L)
            assertTrue(processDiagnostics.memory.graphicsPssBytes >= 0L)
            assertTrue(processDiagnostics.memory.runtimeMaxMemoryBytes > 0L)
            assertTrue(processDiagnostics.memory.processCpuTimeMs >= 0L)
            assertTrue(processDiagnostics.memory.oomScoreAdj != null)
