...
That correctness roadmap has priority over the completed Phase 5 through
Phase 7 behavior recorded here, including historical tests that allowed an
old-model run or playback session to survive an active-model change or allowed
Cache Management to play an inactive model's completed entry directly.
...
> Historical contract notice: clauses below that retain an old playback
> session across an active-model change or directly play an inactive model's
> cache are superseded by the dedicated lifecycle/cache correctness roadmap.
> They remain here only as a record of the Phase 5 implementation baseline.
...
> Historical implementation record: active-model handoff and cache playback
> semantics in this completed phase are being replaced by the dedicated
> lifecycle/cache correctness roadmap. The immutable cache/run identity and
> exact-entry lease foundations remain valid.
...
> Historical implementation record: direct completed-cache playback and
> retaining an old separated session across active-model changes are no longer
> product requirements. See the dedicated lifecycle/cache correctness roadmap.
